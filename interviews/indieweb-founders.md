...
**Manton:** And that was using [Webmention](https://indieweb.org/Webmention)? 

**Aaron:** Yeah, exactly. 

**Manton:** So back in 2002 what was the technology to.. It was way after [Pingbacks](https://indieweb.org/pingback) and [Trackbacks](https://en.wikipedia.org/wiki/Trackback).

**Aaron:** Yeah, the thought was the [OStatus](https://www.w3.org/community/ostatus/wiki/Main_Page) stack. [Atom](https://tools.ietf.org/html/rfc4287). [Salmon](https://en.wikipedia.org/wiki/Salmon_(protocol)) for the responses. 

**Tantek:** [PubSubHubbub](https://github.com/pubsubhubbub/), I think. 
...
**Aaron:** Mastodon ripped out their whole code and replaced it with [ActivityPub](https://www.w3.org/TR/activitypub/). So after that event we were like, "okay, that was cool, but we want to focus on people being more empowered individually to participate in this federated network." 
...
**Manton:** And the first one, importantly... The very first principle of the [IndieWeb principles](https://indieweb.org/principles) is that. Use your own site, own your own content. Domain names are a big part of that. Really the biggest thing. Unfortunately domain names have not changed since 2010. They haven't changed since 2000. It's interesting that something so important, that "step one" is still really confusing to people. 
...
**Manton:** I'm going with: why aren't they easier? Because a lot of what the IndieWeb does, if you look at [Microformats](http://microformats.org) and [Micropub](https://www.w3.org/TR/micropub/) and all these core parts of the IndieWeb, they're built on HTML and HTTP where we have some control over making things easier and having other standards. 
...
**Tantek:** Other hosting providers have made it easier to one-key turn on WordPress, turn on [Known](https://withknown.com). I don't know there's a domain name registrar that also has easy hosting that also has a simple turn on a web site CMS. 

**Aaron:** DreamHost does a pretty good job of it. 

**Tantek:** That's true. DreamHost probably has the most pieces that they've put together. And they were the first to do [LetsEncrypt](https://letsencrypt.org) automation. Domain name, security, hosting. So that's evolved. It's very impressive to watch, even during the whole eight years of IndieWebCamp.
